...
db_mask = deband_detail_mask(final_dn)
db_mask = Morpho.inflate(db_mask, radius=4)
db = f3k_deband(final_dn, radius=20, thr=110, grain=0.0, dynamic_grain=False, planes=[0])
db = f3k_deband(db, radius=10, thr=220, grain=0.0, dynamic_grain=False, planes=[0])
db = f3k_deband(db, radius=5, thr=220, grain=0.40, dynamic_grain=False, planes=[0])
db = limit_filter(db, final_dn, dark_thr=0.35, bright_thr=0.70, elast=2.5, planes=[0])
db = core.std.MaskedMerge(db, final_dn, db_mask)
